...
// Function to upload image to cloudinary but keep local copy
const uploadToCloudinary = async (imagePath) => {
  try {
    ...
    // Note: We're NOT deleting the local file as we're keeping both copies
    
    return {
      url: result.secure_url,
      publicId: result.public_id,
      localPath: imagePath,
      localUrl: `/uploads/${path.basename(imagePath)}`
    };
  } catch (error) {
...
export const startNewChat = async (userId, prompt, imagePath = null) => {
  ...
  // Step 1: Upload image to Cloudinary if provided (but keep local copy)
  if (imagePath) {
  ...
  // Step 2: Create user message with both URLs
  const userMessage = await Message.create({
    chatId,
    sender: "user",
    text: prompt,
    image: imageData.cloudinaryUrl,
    localImage: imageData.localUrl
  });
...
// Upload image to Cloudinary and keep local copy (for the separate upload endpoint)
