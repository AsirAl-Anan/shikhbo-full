...
function NewChatPage() {
    ...
    if (paramChatId !== undefined && paramChatId !== null && paramChatId !== "null") { 
      const fetchChat = async () => {
        ...
        try {
          ...
          localStorage.removeItem('chatModalQuery');
          localStorage.removeItem('chatModalPending');
          localStorage.removeItem('lastUserMessage'); // Also clear any cached messages
          pendingQueryProcessed.current = false;
        } catch (error) {
  ...
  useEffect(() => {
    ...
    socketRef.current.on("chatStarted", ({ chatId, chatName, messages: newMessages }) => {
      ...
      // Store the last user message in case it's not included in response
      const lastUserMessage = localStorage.getItem('lastUserMessage');
      
      // Create a merged message array that preserves the user message
      const mergedMessages = [...newMessages]; // Start with server messages
      
      if (lastUserMessage) {
        const userMessageObj = JSON.parse(lastUserMessage);
        
        // Check if user message is already in the new messages
        const userMessageExists = newMessages.some(msg => 
          msg.role === "user" && msg.content === userMessageObj.content
        );
        
        if (!userMessageExists && userMessageObj.content) {
          // If not, add it to ensure it's visible
          // Find the right position for the user message (before AI response)
          const aiIndex = newMessages.findIndex(msg => msg.role === "assistant");
          if (aiIndex > -1) {
            mergedMessages.splice(aiIndex, 0, userMessageObj);
          } else {
            mergedMessages.unshift(userMessageObj);
          }
        }
      ...
      messagesRef.current = mergedMessages; // Update ref
    ...
    socketRef.current.on("messageReceived", ({ chatId: receivedChatId, chatName: receivedChatName, messages: receivedMessages }) => {
      ...
      // Get cached user message if it exists
      const lastUserMessage = localStorage.getItem('lastUserMessage');
      let userMessageObj = null;
      
      if (lastUserMessage) {
        userMessageObj = JSON.parse(lastUserMessage);
      }
      ...
      if (receivedChatId && receivedChatId === paramChatId) {
        ...
        // Check if we need to add the cached user message
        if (userMessageObj) {
          const userMessageExists = newMessages.some(msg => 
            (msg.role === "user" && msg.content === userMessageObj.content) ||
            receivedMessages.some(rMsg => rMsg.role === "user" && rMsg.content === userMessageObj.content)
          );
        ...
        // Clean up the cached user message
        localStorage.removeItem('lastUserMessage');
        ...
        const mergedMessages = [...receivedMessages];
        
        // Add the cached user message if it's not already included
        if (userMessageObj) {
          const userMessageExists = receivedMessages.some(msg => 
            msg.role === "user" && msg.content === userMessageObj.content
          );
          
          if (!userMessageExists && userMessageObj.content) {
            // Find proper position for user message (before AI response)
            const aiIndex = receivedMessages.findIndex(msg => msg.role === "assistant");
            if (aiIndex > -1) {
              mergedMessages.splice(aiIndex, 0, userMessageObj);
            } else {
              mergedMessages.unshift(userMessageObj);
  ...
  const handleSendMessage = (e) => {
    ...
    localStorage.setItem('lastUserMessage', JSON.stringify(newUserMessage));
