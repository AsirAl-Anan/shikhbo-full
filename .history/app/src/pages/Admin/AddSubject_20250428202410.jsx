...
const AddSubjectController = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('/api/subjects', { name: subjectName, chapters });
      setSubjectName('');
      setChapters([{ chapterName: '', topics: [''] }]);
      fetchSubjects();
    } catch (error) {
      console.error(error);
    }
            ...
            <div key={subject._id} className="bg-gray-800 p-4 rounded-lg space-y-4 shadow-md">
              ...
              <div className="pl-4 space-y-2">
                {subject.chapters.map((chapter, idx) => (
                  <div key={idx}>
                    <p className="text-lg font-semibold">{chapter.chapterName}</p>
