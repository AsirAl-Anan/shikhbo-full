...
function NewChatPage() {
  ...
  useEffect(() => {
    ...
    // Listening for next messages - FIX: Only append new message instead of replacing all messages
    socketRef.current.on("messageReceived", ({ messages: receivedMessages }) => {
      // Check if we received the full chat history or just the new message
      if (receivedMessages.length === 1) {
        // Only one message received, it's the new one
        setMessages(prevMessages => [...prevMessages, receivedMessages[0]]);
      } else {
        // Full chat history received, use it (fallback)
