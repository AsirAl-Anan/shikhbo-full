...
export function ChatMessage({ message }) {
    const isUser = message[0]?.role === "user"
    const [userMsg, aiMsg] = message
    return (
      <div className={`flex ${userMsg ? "justify-end" : "justify-start"}`}>
        <div className={`max-w-[80%] ${userMsg ? "order-2" : "order-1"}`}>
          {!userMsg && (
            <div className="h-8 w-8 rounded-full bg-gray-600 flex items-center justify-center mb-2">
              <span className="text-xs">AI</span>
            </div>
          )}
          <div className={`rounded-lg p-3 ${userMsg ? "bg-gray-700" : "bg-gray-800"}`}>
