...
const Sidebar = ({ isSidebarOpen, isMobile, toggleSidebar, currentUser }) => {
  ...
  const [chatHistory, setChatHistory] = useState([
    { id: 1, title: "What can Artificial Intelligence.", path: "/chat/1" },
    { id: 2, title: "Generate a script for a 30-second comment...", path: "/chat/2" },
    { id: 3, title: "What stages does the audit include?", path: "/chat/3" },
    { id: 4, title: "Write a script for a training video on how to u...", path: "/chat/4" },
    { id: 5, title: "Generate a summary of our company's missio...", path: "/chat/5" },
    { id: 6, title: "How can I reduce stress at work in the office lo...", path: "/chat/6" },
    { id: 7, title: "TalkTech Nexus : Where Ideas Spark and Con...", path: "/chat/7" },
    { id: 8, title: "Tell me what is Artificial Intelligence", path: "/chat/8" },
  ])
  
  const [chats, setChatResponse] = useState([])
  useEffect(() => {
    const getchatChats = async () => {
      try {
        const response = await instance.get('/ai/chat/getAllchat');
        setChatHistory(response.data.chats);  // Set the state here with the resolved data
      } catch (error) {
        console.error("Error fetching chats:", error);
      }
    };
  
    getchatChats();  // Call the async function to fetch data
  }, []); // Empty dependency array to run once when the component mounts
  
  console.log("chats", chatHistory); // This will log the updated value of chats after it's set
  
 
  const pinnedChats = [
    { id: 1, title: "Generate a script for a 30-second comment...", path: "/chat/2" },
    { id: 2, title: "Tell me what is Artificial Intelligence", path: "/chat/8" },
  ]
      ...
      <div className={`w-64 flex flex-col h-full ${(!isSidebarOpen && !isMobile) ? 'invisible' : 'visible'}`}>
        ...
        <nav className="p-4 space-y-6 flex-1 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            {/* Question Bank */}
            <NavLink to="/questionbank" className={({isActive}) => 
              isActive 
                ? "flex items-center space-x-2 p-2 rounded-md bg-gradient-to-r from-blue-600 to-indigo-700 text-white"
                : "flex items-center space-x-2 p-2 text-gray-300 hover:text-white hover:bg-gray-800 rounded-md"
            }>
            ...
            <div className="space-y-2">
              {pinnedChats.map((chat) => (
                <NavLink 
                  ...
                  className={({isActive}) => 
                    isActive 
                      ? "block text-sm text-white font-medium"
              ...
              <div>
                {
                  chatHistory.map((chat)=>{
                    return(
                      <div key={chat._id} className="flex items-center justify-between p-2 hover:bg-gray-700 rounded-md">
                        <NavLink to={`/chat/${chat._id}`} className="text-sm text-gray-300 hover:text-white">
                          {chat.title}
                        </NavLink>
                        <button onClick={() => handleDeleteChat(chat._id)} className="text-red-500 hover:text-red-400">
          ...
          <div className="relative" ref={settingsDropdownRef}>
            ...
            {showSettingsDropdown && (
              <div className="absolute bottom-10 right-0 w-36 bg-gray-800 rounded-md shadow-lg overflow-hidden z-50">
                <div className="py-1">
                  <NavLink to="/settings" className="block px-4 py-2 text-sm text-gray-200 hover:bg-gray-700">
                    Settings
                  </NavLink>
                  <button 
